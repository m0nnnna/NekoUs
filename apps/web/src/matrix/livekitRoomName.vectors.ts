 ...
 *
 * The function is implemented twice — here in `voice.ts` (btoa) and in
 * `services/token-server/src/livekitRoomName.ts` (Buffer) — because the two packages don't share
 * one, and a token minted for a name the client doesn't use produces a call where nobody can
 * hear anyone and nothing anywhere reports an error. "Keep these byte-identical" was only ever a
 * comment, which is not a thing that fails when someone stops doing it.
 *
 * **This file is duplicated verbatim at
 * `services/token-server/src/livekitRoomName.vectors.ts`, and each package asserts its own
 * implementation against it.** Duplicating the expectations rather than the reasoning is the
 * point: a change to either implementation now has to be matched in both copies of this list to
 * go green, which is exactly the moment someone should notice the other implementation exists.
 */
